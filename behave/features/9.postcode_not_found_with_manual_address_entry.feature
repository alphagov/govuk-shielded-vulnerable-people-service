Feature: COVID-19 Shielded vulnerable people service - partial user journey - postcode not found after change address (user lives in England)
    ...
    Scenario: Should be directed to support address when the my address isn't listed link is clicked
    ...
    Scenario: Should be re-directed to address lookup if user lives in England
